...
void task_veml7700_read(void *ignore)
{
    ...
    while (true) {
        ...
        ESP_ERROR_CHECK(veml7700_read_als_lux_auto(TCA9548_ADDR_2, &lux_als_2));
        fc_als_2 = lux_als_2 * LUX_FC_COEFFICIENT;

        ESP_ERROR_CHECK(veml7700_read_white_lux_auto(TCA9548_ADDR_2, &lux_white_2));
        fc_white_2 = lux_white_2 * LUX_FC_COEFFICIENT;

        // Read data from the second sensor (Channel 3)
        ESP_ERROR_CHECK(veml7700_read_als_lux_auto(TCA9548_ADDR_3, &lux_als_3));
        fc_als_3 = lux_als_3 * LUX_FC_COEFFICIENT;

        ESP_ERROR_CHECK(veml7700_read_white_lux_auto(TCA9548_ADDR_3, &lux_white_3));
